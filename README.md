...
- 真实手机浏览器使用原生纵向滚动，操作手感与系统浏览器保持一致
- 移动端底部“保存改善”栏固定显示在滚动区域之外，表单内容会自动预留安全空间
- 宽屏设备同时提供电脑端台账与移动端录入面板
...
手机视口下，应用不显示原型设备边框、模拟状态栏或模拟键盘，而是直接占满浏览器可视区域。内容采用浏览器原生滚动，保存栏作为固定的独立界面层，避免在填写或滚动时误触后跳转到页面底部。

语音输入不作为浏览器端的录入入口；可直接使用系统键盘填写现场描述，再通过“智能整理表述”生成规范的改善内容和效果。
